...
class MyGLViewer(GLSimulationPlugin):
    ...
    def control_loop(self):
        ...
        Contact_Force_Get(self)

        flag_t, qddot_t, beta_t, lamda_t, tau_t = QP_Controller(self)
...
class Robot_Init_Opt_Prob(probFun):
    ...
    def __init__(self, world, sim_robot, sigma_t, state_init):
        ...
        nx = len(state_init)
        y_val, y_type = Initial_Constraint_Eqn(sigma_t, state_init, sim_robot, world, state_init)
        nc = len(y_type)
        probFun.__init__(self, nx, nc)

    def __callf__(self, x, y):
        y_val, y_type = Initial_Constraint_Eqn(self.sigma_t, self.state_init, self.sim_robot, self.world, x)
        # ipdb.set_trace()
...
def Initial_Constraint_Eqn(sigma_t, state_init, sim_robot, world, x):
...
def Robot_Init_Opt_fn(world, sigma_t, state_init):
    ...
    Robot_Init_Opt = Robot_Init_Opt_Prob(world, sim_robot, sigma_t, state_init)
...
def main():
    ...
    sigma_t = [1,1,0,0]                      # This is the initial contact status:  1__------> contact constraint is active,
    ...
    Init_Config, Init_Velocity = Robot_Init_Opt_fn(world, sigma_t, State_Init)
    ...
    viewer = MyGLViewer(world, Init_Config, Init_Velocity, sigma_t)
...
def Contact_Force_Get(self):
    #  This function is uised to get the contact force from the current simulation
    exp_simulator = Simulator(self.world)
    # The next job is to figure out what is the current contact point

    print exp_simulator.getActualConfig(0)


def Contact_Stability_Wrench_Plot(self):
    # This function is used to plot the contact wrench given the current robot
    # Instead of giving one specific plot, here we would like to plot the possible space that the contact wrench could occupy
    sim_robot = self.sim_robot

    # The plot is drawn with respect to the center of mass of the robot
    Sigma_Status = self.sigma_t

    robot_COM = sim_robot.getCom()  # Here the robot center of mass position is retrieved

    # Then the next job is to compute the relative position from the robot end effector to the center of mass

    Left_Foot_Link = sim_robot.link(End_Link_Ind[0])
    Left_Foot_Link_Pos1 = Left_Foot_Link.getWorldPosition(Left_Foot_End[0])
    Left_Foot_Link_Pos2 = Left_Foot_Link.getWorldPosition(Left_Foot_End[1])
    Left_Foot_Link_Pos3 = Left_Foot_Link.getWorldPosition(Left_Foot_End[2])
    Left_Foot_Link_Pos4 = Left_Foot_Link.getWorldPosition(Left_Foot_End[3])

    Right_Foot_Link = sim_robot.link(End_Link_Ind[1])
    Right_Foot_Link_Pos1 = Right_Foot_Link.getWorldPosition(Right_Foot_End[0])
    Right_Foot_Link_Pos2 = Right_Foot_Link.getWorldPosition(Right_Foot_End[1])
    Right_Foot_Link_Pos3 = Right_Foot_Link.getWorldPosition(Right_Foot_End[2])
    Right_Foot_Link_Pos4 = Right_Foot_Link.getWorldPosition(Right_Foot_End[3])

    Left_Hand_Link = sim_robot.link(End_Link_Ind[2])
    Left_Hand_Link_Pos = Left_Hand_Link.getWorldPosition(Left_Hand_End[:])

    Right_Hand_Link = sim_robot.link(End_Link_Ind[3])
    Right_Hand_Link_Pos = Right_Hand_Link.getWorldPosition(Right_Hand_End[:])
    

    # The idea to get the vertex of the convex of the whole system is a little hard to conduct
